// ATENÇÃO: Verifique a URL do seu Web App
        ...
        // GID_ATUAL é inicializado como null e só será setado em iniciarTabs se não for válido
        let GID_ATUAL = null; 
        ...
        // --- LÓGICA DE SINCRONIZAÇÃO ENTRE ABAS (BroadcastChannel) ---
        const SYNC_CHANNEL_NAME = 'educacao_continuada_sync';
        const statusChannel = new BroadcastChannel(SYNC_CHANNEL_NAME);

        statusChannel.onmessage = (event) => {
            const data = event.data;
            const rowElement = document.querySelector(`tr[data-row-index="${data.rowIndex}"]`);

            if (data.action === 'lock_item' && rowElement) {

                const colaboradorTd = getColaboradorTd(rowElement);
                const statusSelect = rowElement.querySelector('td[data-col="status"] select');

                // Garante que a linha tenha o colaborador e o status "EM ANDAMENTO"
                if (statusSelect.value.toUpperCase() !== 'EM ANDAMENTO') {
                    statusSelect.value = 'EM ANDAMENTO';
                }
                if (colaboradorTd.textContent.trim() === '') {
                    colaboradorTd.textContent = data.colaborador;
                }
                
                // Aplica os bloqueios/visibilidade
                toggleFormularioDisabled(rowElement, false); 
                toggleObservacaoDisabled(rowElement, false);
                rowElement.classList.add('andamento-ativo');

                // Força o bloqueio de status para a opção "EM ANDAMENTO" e "CONCLUÍDO"
                ...
                // Salva o valor anterior para evitar o modal novamente
                statusSelect.setAttribute('data-prev-value', 'EM ANDAMENTO');

                console.log(`[Broadcast] Item ${data.rowIndex} bloqueado e em andamento por outra aba.`);
            }
        };
        ...
        async function salvarDadosPlanilha(rowElement) {
            ...
            // INÍCIO DO AJUSTE: Parar o Polling antes de iniciar a operação assíncrona
            pararPolling(); 
            ...
            try {
                ...
                // FIM DO AJUSTE: Reiniciar o Polling após a conclusão da operação (sucesso ou falha)
                iniciarPolling(); 
            }
        }
        // -----------------------------------------------------
        ...
        function salvarColaborador() {
            ...
            if (novoNome) {
                ...
                toggleObservacaoDisabled(row, false); // Habilita o campo de observação
                ...
                // --- FIM ATUALIZAÇÃO VISUAL INSTANTÂNEA ---


                // NOVO: Envia a mensagem de bloqueio para outras abas (instantâneo)
                statusChannel.postMessage({
                    action: 'lock_item',
                    rowIndex: rowIndex,
                    colaborador: novoNome
                });

                // CHAMA O SALVAMENTO (Assíncrono, que agora gerencia o Polling)
                salvarDadosPlanilha(row);
                
                // REMOVIDA A CHAMADA REDUNDANTE: gerenciarVisibilidadeColunas(statusSelect);
        ...
        function gerenciarVisibilidadeColunas(selectElement) {
            ...
            if (valorAtual === 'EM ANDAMENTO') {
                ...
                if (!colaboradorPreenchido) {
                    abrirModal(selectElement); // ABRE O MODAL PARA OBTER O NOME
                    return; // SAI DA FUNÇÃO, O SALVAMENTO SERÁ FEITO EM salvarColaborador()
                } else {
                    row.classList.add('andamento-ativo');
                    // Habilita o campo de observação
                    toggleObservacaoDisabled(row, false);
                    salvarDadosPlanilha(row); // Salva a mudança (mantendo EM ANDAMENTO, ou quando o filtro foi aplicado manualmente)
                }
                ...
                toggleObservacaoDisabled(row, true); // Bloqueia a observação também
                salvarDadosPlanilha(row);
                ...
                toggleObservacaoDisabled(row, true); // Garante que a observação está desabilitada
                salvarDadosPlanilha(row);
            }
        ...
        async function iniciarTabs() {
            ...
            // CORREÇÃO: Somente define GID_ATUAL se ele não for um GID válido (mantendo a aba atual)
            if (!Object.values(FONTES_DADOS).includes(GID_ATUAL)) {
            ...
            Object.keys(FONTES_DADOS).forEach(nome => {
                ...
                button.addEventListener('click', () => {
                    ...
                    // NOVO: Parar e reiniciar o Polling durante a troca de aba
                    pararPolling();
                    carregarDadosPlanilha(GID_ATUAL).finally(() => {
                        iniciarPolling();
                    });
                });
            ...
            // Só carrega os dados se o GID_ATUAL for definido 
            if(GID_ATUAL) {
        ...
        function openFilterPopup(columnId, headerElement) {
            closeFilterPopup(); // Fechá qualquer filtro anterior
        ...
        function iniciarPolling() {
            ...
            if (idIntervaloAtualizacao !== null) {

                return;
            }

            idIntervaloAtualizacao = setInterval(() => {

                if (typeof iniciarTabs === 'function') {
                    iniciarTabs();
        ...
        function pararPolling() {
            if (idIntervaloAtualizacao !== null) {
                clearInterval(idIntervaloAtualizacao);
                idIntervaloAtualizacao = null;
